...
{
    public class ContractLotBL : Controller
    {
        private ContractLotDAL contractDAL = new ContractLotDAL();
        ...
        public MemoryStream GeneratePreProcurementMemo(string ContractCode, string LogoPath, string UserEmail)
        {
            ...
            for (var i = 0; i < preProcurementOffices.Count; i++)
            {
                ...
                reports.AddFormattedRowContent(new ContentCell(new TextWithFormat[]
                {
                    ...
                    new TextWithFormat(preProcurementOffices[i].Designation + " " + preProcurementOffices[i].DepartmentHeadLastName, true, true, 10),
                ...
                reports.AddFormattedRowContent(new ContentCell(new TextWithFormat[]
                {
                    ...
                    new TextWithFormat(" - a LOT BIDDING, on the following schedule:", false, false, 10)
                ...
                rows = new List<ContentCell>();
                rows.Add(new ContentCell("Lot No.", 0, 10, true, false, ParagraphAlignment.Center, VerticalAlignment.Center));
                rows.Add(new ContentCell("Procurement Project", 1, 10, true, false, ParagraphAlignment.Center, VerticalAlignment.Center));
                rows.Add(new ContentCell("Date and Venue", 2, 10, true, false, ParagraphAlignment.Center, VerticalAlignment.Center));
                reports.AddRowContent(rows, 0.20);

                var count = 1;
                foreach(var detail in preProcurementOffices[i].LotDetails)
                {
                    rows = new List<ContentCell>();
                    rows.Add(new ContentCell(count.ToString(), 0, 10, false, false, ParagraphAlignment.Center, VerticalAlignment.Top));
                    rows.Add(new ContentCell(detail.ContractName + "\n(" + detail.ContractCode + ")", 1, 10, false, false, ParagraphAlignment.Left, VerticalAlignment.Top));
                    rows.Add(new ContentCell(detail.PreProcurementConferenceDate.Value.ToString("dd MMMM yyyy hh:mm tt") + "\n" + detail.PreProcurementConferenceLocation, 2, 10, false, false, ParagraphAlignment.Left, VerticalAlignment.Top));
                    reports.AddRowContent(rows, 0.20);
                    count++;
                }
        ...
        public MemoryStream GenerateInvitationToBid(string ContractCode, string LogoPath, string UserEmail)
        {
            ...
            reports.AddColumnHeader(
                new HeaderLine { Content = bidsAndAwards.Sector, Bold = false, Italic = false, FontSize = 8.5, ParagraphAlignment = ParagraphAlignment.Left },
                new HeaderLine { Content = "", Bold = false, Italic = false, FontSize = 7, ParagraphAlignment = ParagraphAlignment.Left }
            );
            reports.AddColumnHeader(
                new HeaderLine { Content = bidsAndAwards.Section.ToUpper(), Bold = true, Italic = false, FontSize = 9, ParagraphAlignment = ParagraphAlignment.Left },
                new HeaderLine { Content = "", Bold = true, Italic = false, FontSize = 8, ParagraphAlignment = ParagraphAlignment.Left }
            );
            reports.AddColumnHeader(
            ...
            string fundSources = string.Empty;
            for(int i = 0; i < invitationToBid.Details.Count; i++)
            {
                if(i == invitationToBid.Details.Count - 1)
                {
                    fundSources += invitationToBid.Details[i].ApprovedBudgetForContractWords + " (" + invitationToBid.Details[i].ApprovedBudgetForContract.ToString("C", new System.Globalization.CultureInfo("en-ph")) + ")";
                }
                else
                {
                    fundSources += invitationToBid.Details[i].ApprovedBudgetForContractWords + " (" + invitationToBid.Details[i].ApprovedBudgetForContract.ToString("C", new System.Globalization.CultureInfo("en-ph")) + "); ";
            ...
            //    new TextWithFormat(invitationToBid.Details.ForEach(d => d.FundDescription), true, false, 10),
        ...
        public MemoryStream GenerateNoticeOfAward(string ContractCode, string LogoPath, string UserEmail)
        {
            ...
            rows.Add(new ContentCell(new TextWithFormat[]
            {
                ...
                new TextWithFormat(" through " + noticeOfAward.ModeOfProcurement + " in the amount of ", false, false, 10),
                new TextWithFormat(noticeOfAward.ContractPriceWords + " (" + noticeOfAward.ContractPrice.ToString("C", new System.Globalization.CultureInfo("en-ph")) + ")", true, true, 10),
                new TextWithFormat(" to " + noticeOfAward.Supplier + ".", false, false, 10)
            }, 0, ParagraphAlignment.Justify, VerticalAlignment.Top));
            reports.AddRowContent(rows, 0.25);
        ...
        public MemoryStream GenerateContractAgreementFormGoods(string ContractCode, string LogoPath, string UserEmail)
        {
            ...
            rows.Add(new ContentCell(new TextWithFormat[]
            {
                ...
                new TextWithFormat(noticeOfAward.Supplier, true, true, 10),
                new TextWithFormat(" of  ", false, false, 10),
                new TextWithFormat(noticeOfAward.State, false, true, 10),
            ...
            rows.Add(new ContentCell(new TextWithFormat[]
            {
                ...
                new TextWithFormat(noticeOfAward.ContractName, false, true, 10),
                ...
                new TextWithFormat(noticeOfAward.ContractPriceWords + " (" + noticeOfAward.ContractPrice.ToString("C", new System.Globalization.CultureInfo("en-ph")) + ")", false, true, 10),
        ...
        {
            Reports reports = new Reports();
            var contract = contractDAL.PurchaseOrderSetup(ContractCode);
            reports.ReportFilename = contract.PurchaseOrderNumber;
        ...
        public MemoryStream GenerateNoticeToProceed(string ContractCode, string LogoPath, string UserEmail)
        {
            ...
            var bacsecHead = db.BACSecretariat.Where(d => d.Membership == BACMembership.BACSECHead && d.PurgeFlag == false).FirstOrDefault();
            var bacChair = db.BACSecretariat.Where(d => d.Membership == BACMembership.BACChairman && d.PurgeFlag == false).FirstOrDefault();
            var noticeOfAward = contractDAL.NoticeToProceedSetup(ContractCode);
            Reports reports = new Reports();
    ...
    public class ContractLotDAL : Controller
    {
        private FMISDbContext db = new FMISDbContext();
        private HRISDataAccess hris = new HRISDataAccess();
        ...
        public PreProcurementConferenceLotSetupVM GetPurchaseRequestSubmissionClosing(string ContractCode)
        {
            var lotPreProcurementDetails = db.ProcurementProjects.Where(d => d.FKParentProjectReference.ContractCode == ContractCode).ToList()
                .Select(d => new PreProcurementConferenceLotSetupDetailsVM
                {
            ...
            var contract = db.ProcurementProjects.ToList().Where(d => d.ContractCode == ContractCode).Select(d => new PreProcurementConferenceLotSetupVM
            {
                ...
                Details = lotPreProcurementDetails,
        ...
        public PreProcurementConferenceLotUpdateVM GetPreProcurementConferenceUpdate(string ContractCode)
        {
            var lotPreProcurementDetails = db.ProcurementProjects.Where(d => d.FKParentProjectReference.ContractCode == ContractCode).ToList()
                .Select(d => new PreProcurementConferenceLotUpdateDetailsVM {
                    ContractCode = d.ContractCode,
                    ContractName = d.ContractName,
                    ApprovedBudgetForContract = d.ApprovedBudgetForContract,
                    DeliveryPeriod = d.DeliveryPeriod.Value,
            ...
            var contract = db.ProcurementProjects.ToList().Where(d => d.ContractCode == ContractCode).Select(d => new PreProcurementConferenceLotUpdateVM {
                PAPCode = d.FKAPPReference.PAPCode,
                ClassificationReference = d.ClassificationReference,
                Classification = d.FKClassificationReference.GeneralClass + " - " + d.FKClassificationReference.Classification,
                ModeOfProcurementReference = d.ModeOfProcurementReference,
        ...
        public PreBidConferenceLotSetupVM GetPreBidConferenceSetup(string ContractCode)
        {
            var lotPreBidDetails = db.ProcurementProjects.Where(d => d.FKParentProjectReference.ContractCode == ContractCode).ToList()
            .Select(d => new PreBidConferenceLotSetupDetailsVM
            {
                ContractCode = d.ContractCode,
                ContractName = d.ContractName,
                ApprovedBudgetForContract = d.ApprovedBudgetForContract,
                ...
                OpeningOfBids = d.OpeningOfBids_OpeningOfQuotations,
                DeadlineOfSubmissionOfBids = d.DeadlineOfSubmissionOfBids_DeadlineOfSubmisionOfBids,
                ContractItems = contractDAL.GetProcurementProjectItems(d.ContractCode)
            }).ToList();

            var contract = db.ProcurementProjects.ToList().Where(d => d.ContractCode == ContractCode).Select(d => new PreBidConferenceLotSetupVM
            {
                PAPCode = d.FKAPPReference.PAPCode,
                ClassificationReference = d.ClassificationReference,
                Classification = d.FKClassificationReference.GeneralClass + " - " + d.FKClassificationReference.Classification,
        ...
        public PreBidConferenceLotUpdateVM GetPreBidConferenceUpdate(string ContractCode)
        {
            var lotPreBidDetails = db.ProcurementProjects.Where(d => d.FKParentProjectReference.ContractCode == ContractCode).ToList()
            .Select(d => new PreBidConferenceLotUpdateDetailsVM
            {
                ContractCode = d.ContractCode,
                ContractName = d.ContractName,
                ApprovedBudgetForContract = d.ApprovedBudgetForContract,
            ...
            var contract = db.ProcurementProjects.ToList().Where(d => d.ContractCode == ContractCode).Select(d => new PreBidConferenceLotUpdateVM
            {
                PAPCode = d.FKAPPReference.PAPCode,
                ClassificationReference = d.ClassificationReference,
                Classification = d.FKClassificationReference.GeneralClass + " - " + d.FKClassificationReference.Classification,
        ...
        public bool PostPreProcurementConferenceSetup(PreProcurementConferenceLotSetupVM Contract, string UserEmail)
        {
            ...
            var childContracts = db.ProcurementProjects.Where(d => d.ParentProjectReference == contract.ID).ToList();
            ...
            db.ContractUpdates.Add(new ContractUpdates
            {
                ...
                Remarks = "Pre-Bid Conference Conference for " + Contract.ContractCode + " is set up. Additional Information may be seen on the next updates (per lot)."
            });

            for (int i = 0; i < childContracts.Count; i++)
            {
                childContracts[i].PreProcurementConferenceLocation = Contract.Details[i].PreProcurementConferenceLocation;
                childContracts[i].ProcurementProjectStage = ProcurementProjectStages.PreProcurementConferenceSetup;
                db.ContractUpdates.Add(new ContractUpdates
                {
                    ProcurementProjectReference = contract.ID,
                    ProcurementProjectStage = ProcurementProjectStages.PreProcurementConferenceSetup,
                    UpdatedAt = DateTime.Now,
                    UpdatedBy = user.EmpCode,
                    Remarks = "Pre-Procurement Conference for " + childContracts[i].ContractCode + " is set up. Memorandum for End-Users/Implementing Units is now available for printing."
                });
            }
        ...
        public bool PostPreProcurementConferenceUpdate(PreProcurementConferenceLotUpdateVM Contract, string UserEmail)
        {
            var user = db.UserAccounts.Where(d => d.Email == UserEmail).FirstOrDefault();
            var contract = db.ProcurementProjects.Where(d => d.ContractCode == Contract.ContractCode).FirstOrDefault();
            var childContracts = db.ProcurementProjects.Where(d => d.FKParentProjectReference.ContractCode == Contract.ContractCode).ToList();
            ...
            db.ContractUpdates.Add(new ContractUpdates
            {
                ...
                Remarks = "Pre-Procurement Conference for " + Contract.ContractCode + " was updated. Additional Information may be seen on the next updates (per lot)."
            });

            for (int i = 0; i < childContracts.Count; i++)
            {
                childContracts[i].ProcurementProjectStage = ProcurementProjectStages.PreProcurementConferenceUpdate;
                db.ContractUpdates.Add(new ContractUpdates
                {
                    ProcurementProjectReference = contract.ID,
                    ProcurementProjectStage = ProcurementProjectStages.PreProcurementConferenceUpdate,
                    UpdatedAt = DateTime.Now,
                    UpdatedBy = user.EmpCode,
                    AccomplishedAt = Contract.Details[i].ContractUpdate.AccomplishedAt,
                    Remarks = "Pre-Procurement Conference for " + Contract.ContractCode + " was updated. Additional Information: \n" + Contract.Details[i].ContractUpdate.Remarks
                });
            }
        ...
        public bool PostPreBidConferenceSetup(PreBidConferenceLotSetupVM Contract, string UserEmail)
        {
            var user = db.UserAccounts.Where(d => d.Email == UserEmail).FirstOrDefault();
            var contract = db.ProcurementProjects.Where(d => d.ContractCode == Contract.ContractCode).FirstOrDefault();
            var childContracts = db.ProcurementProjects.Where(d => d.FKParentProjectReference.ContractCode == Contract.ContractCode).ToList();
            ...
            for (int i = 0; i < childContracts.Count; i++)
            {
                var details = Contract.Details.Where(d => d.ContractCode == childContracts[i].ContractCode).FirstOrDefault();
                if (Contract.ProcurementProjectType == ProcurementProjectTypes.CPB)
                {
                    childContracts[i].ProcurementProjectStage = ProcurementProjectStages.PreBidConferenceSetup;
                    childContracts[i].PreBidConferenceLocation = details.PreBidConferenceLocation;
                    childContracts[i].OpeningOfBidsLocation = details.OpeningOfBidsLocation;
                    childContracts[i].PreBidVideoConferencingOptions = details.PreBidVideoConferencingOptions;
                    childContracts[i].PreBidVideoConferenceMode = details.PreBidVideoConferenceMode;
                    childContracts[i].PreBidVideoConferenceAccessRequestEmail = details.PreBidVideoConferenceAccessRequestEmail;
                    childContracts[i].PreBidVideoConferenceAccessRequestContactNo = details.PreBidVideoConferenceAccessRequestContactNo;
                    childContracts[i].PreBidAdditionalInstructions = details.PreBidAdditionalInstructions;
                    childContracts[i].BidDocumentPrice = details.BidDocumentPrice;
                    db.ContractUpdates.Add(new ContractUpdates
                    {
                        ProcurementProjectReference = contract.ID,
                        ProcurementProjectStage = ProcurementProjectStages.PreBidConferenceSetup,
                        UpdatedAt = DateTime.Now,
                        UpdatedBy = user.EmpCode,
                        Remarks = "Pre-Bid Conference for " + childContracts[i].ContractCode + " is set up. Invitation To Bid is now available for printing."
                    });
                }
                else
                {
                    childContracts[i].ProcurementProjectStage = ProcurementProjectStages.PreBidConferenceSetup;
                    childContracts[i].OpeningOfBidsLocation = details.OpeningOfBidsLocation;
                    childContracts[i].SolicitationNo = GenerateSolicitationNo(Contract.ContractCode);
                    db.ContractUpdates.Add(new ContractUpdates
                    {
                        ProcurementProjectReference = contract.ID,
                        ProcurementProjectStage = ProcurementProjectStages.PreBidConferenceSetup,
                        UpdatedAt = DateTime.Now,
                        UpdatedBy = user.EmpCode,
                        Remarks = "Pre-Bid Conference " + childContracts[i].ContractCode + " is set up. Request for Quotation is now available for printing."
                    });
                }
            }
        ...
        public List<PreProcurementConferenceLotTemplateVM> GetPreProcMemoOffices(string ContractCode, string UserEmail)
        {
            var user = db.UserAccounts.Where(d => d.Email == UserEmail).FirstOrDefault();
            ...
            var ppmpDetails = db.PPMPDetails.ToList().Where(d => d.ProcurementProject != null && childContract.Select(x => x.ContractCode).ToList().Contains(d.FKProcurementProject.ContractCode)).ToList();
            ...
            var preProcurementOffices = new List<PreProcurementConferenceLotTemplateVM>();
            foreach (var endUser in endUsers)
            {
                var office = hris.GetDepartmentDetails(endUser);
                preProcurementOffices.Add(new PreProcurementConferenceLotTemplateVM
                {
                    ...
                    LotDetails = childContract.Select(d => new PreProcurementConferenceLotTemplateDetailsVM {
                        ContractCode = d.ContractCode,
                        ContractName = d.ContractName,
                        PreProcurementConferenceDate = d.PreProcurementConference,
                        PreProcurementConferenceLocation = d.PreProcurementConferenceLocation
                    }).ToList()
                });
            }
        ...
        public InvitationToBidLotTemplateVM InvitationToBidSetup(string ContractCode)
        {
            var details = db.ProcurementProjects.Where(d => d.FKParentProjectReference.ContractCode == ContractCode).Select(d => new InvitationToBidLotTemplateDetailsVM
            {
            ...
            {
                detail.LotNo = "Lot " + lotCount.ToString();
                lotCount++;
            }

            return db.ProcurementProjects.Where(d => d.ContractCode == ContractCode).ToList().Select(d => new InvitationToBidLotTemplateVM
            {
        ...
        public NoticeOfAwardTemplateVM NoticeOfAwardSetup(string ContractCode)
        {
            var procurementProject = db.ProcurementProjects.Where(d => d.ContractCode == ContractCode).FirstOrDefault();
            var supplierBL = new SuppliersBL();
            var contract = db.Contract.Where(d => d.ProcurementProjectReference == procurementProject.ID).FirstOrDefault();
            var supplier = supplierBL.GetSupplierDetails(contract.SupplierReference);
            return new NoticeOfAwardTemplateVM
            {
                Supplier = supplier.SupplierName,
                Address = supplier.Address,
                City = (supplier.City == string.Empty ? string.Empty : supplier.City),
                State = (supplier.State == string.Empty ? string.Empty : supplier.State),
                PostalCode = (supplier.PostalCode == string.Empty ? string.Empty : supplier.PostalCode),
                ...
                ContractCode = procurementProject.ContractCode,
                ContractName = procurementProject.ContractName,
                ContractPrice = contract.ContractPrice,
                ContractPriceWords = Reports.AmountToWords(contract.ContractPrice),
                ModeOfProcurement = procurementProject.FKModeOfProcurementReference.ModeOfProcurementName
            };
        }
        ...
        public NoticeToProceedTemplateVM NoticeToProceedSetup(string ContractCode)
        {
        ...
        public PurchaseOrderTemplateVM PurchaseOrderSetup(string ContractCode)
        {
