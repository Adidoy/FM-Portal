...
namespace PUPFMIS.Controllers
{
    [Route("{action}")]
    [RouteArea("admin")]
    [RoutePrefix("master-tables/item-types")]
    ...
    public class ItemTypesController : Controller
    {
        ...
        public ActionResult Edit(string ItemTypeCode)
        {
            ...
            ItemType itemType = itemTypesDAL.GetItemType(ItemTypeCode, false);
            if (itemType == null)
            {
                return HttpNotFound();
            }
            var InventoryTypes = inventoryTypeDataAccess.GetInventoryTypes().OrderBy(d => d.InventoryTypeName).ToList();
            ViewBag.InventoryTypeReference = new SelectList(InventoryTypes, "ID", "InventoryTypeName", itemType.InventoryTypeReference);
            ViewBag.AccountClass = new SelectList(abisDataAccess.GetDetailedChartOfAccounts().Where(d => d.GenAcctName == "Maintenance and Other Operating Expenses").ToList(), "UACS_Code", "AcctName", itemType.AccountClass);
            return View(itemType);
        }
