...
namespace PUPFMIS.BusinessAndDataLogic
{
    ...
    public class TextWithFormat
    {
        ...
        public TextWithFormat(string Content, bool Bold, bool Italic, double FontSize, bool Superscript)
        {
            TextFont = new Font();
            this.Content = Content;
            TextFont.Bold = Bold;
            TextFont.Italic = Italic;
            TextFont.Size = new Unit(FontSize, UnitType.Point);
            TextFont.Superscript = Superscript;
        }
        public TextWithFormat(string Content, bool Bold, bool Italic, double FontSize, Underline Underline)
        {
            TextFont = new Font();
            this.Content = Content;
            TextFont.Bold = Bold;
    ...
    public class Reports
    {
        ...
        {
            var section = ReportDocument.AddSection();
            section.PageSetup = ReportDocument.LastSection.PageSetup.Clone();
        }
        ...
        public void AddDoubleColumnHeader(string LogoPath, bool DireffentHeaderFirstPage, double HeaderDistance)
        {
            HeaderFooter header = ReportDocument.LastSection.Headers.Primary;
            if (DireffentHeaderFirstPage == true)
            {
                ReportDocument.LastSection.PageSetup.DifferentFirstPageHeaderFooter = true;
                header = ReportDocument.LastSection.Headers.FirstPage;
            }
            header.Section.PageSetup.HeaderDistance = Unit.FromInch(0.25);
            var margins = ReportDocument.LastSection.PageSetup.LeftMargin.Value + ReportDocument.LastSection.PageSetup.RightMargin.Value;
            var pageWidth = ReportDocument.LastSection.PageSetup.Orientation == Orientation.Landscape ? ReportDocument.LastSection.PageSetup.PageHeight.Value - margins : ReportDocument.LastSection.PageSetup.PageWidth.Value - margins;
            var logoWidth = ReportDocument.LastSection.PageSetup.Orientation == Orientation.Landscape ? pageWidth * 0.065 : pageWidth * 0.10;
            var headerWidth = pageWidth - logoWidth;
            ...
            Logo.RelativeVertical = RelativeVertical.Paragraph;
            HeaderTextFrame = HeaderRow.Cells[1].AddTextFrame();
            HeaderTextFrame.Height = 0;

            SubHeaderTable = HeaderTextFrame.AddTable();
            SubHeaderTable.AddColumn(ReportDocument.LastSection.PageSetup.Orientation == Orientation.Portrait ? new Unit((headerWidth * 0.80), UnitType.Inch) : new Unit((headerWidth * 0.65), UnitType.Inch));
            SubHeaderTable.AddColumn(ReportDocument.LastSection.PageSetup.Orientation == Orientation.Portrait ? new Unit((headerWidth * 0.20), UnitType.Inch) : new Unit((headerWidth * 0.35), UnitType.Inch));
            ...
            SubHeaderTable1 = SubHeader1.AddTable();
            SubHeaderTable1.AddColumn(ReportDocument.LastSection.PageSetup.Orientation == Orientation.Portrait ? new Unit((headerWidth * 0.80), UnitType.Inch) : new Unit((headerWidth * 0.60), UnitType.Inch));

            SubHeaderTable2 = SubHeader2.AddTable();
            SubHeaderTable2.AddColumn(ReportDocument.LastSection.PageSetup.Orientation == Orientation.Portrait ? new Unit((headerWidth * 0.20), UnitType.Inch) : new Unit((headerWidth * 0.35), UnitType.Inch));
            ...
            paragraph.Format.Font.Size = new Unit(2.00, UnitType.Point);
            ...
            ReportDocument.LastSection.PageSetup.TopMargin = new Unit(HeaderDistance, UnitType.Inch);
        }
        public void AddPageNumbersFooter()
        {
            HeaderFooter footer = ReportDocument.LastSection.Footers.Primary;
            var paragraph = footer.AddParagraph();
            paragraph.Section.PageSetup.StartingNumber = 1;
            paragraph.AddText("Page ");
            paragraph.AddPageField();
            paragraph.AddText(" of ");
            paragraph.AddNumPagesField();
            footer.Format.Alignment = ParagraphAlignment.Right;
            footer.Format.Font.Size = new Unit(9.00, UnitType.Point);
        }
        /// <summary>
        /// Creates a single column Header for the report document.
        /// </summary>
        ...
        {
            ReportDocument.LastSection.AddParagraph("\n");
        }
        public void AddTable(List<ContentColumn> Columns, bool Bordered)
        {
            ...
            ReportDocument.LastSection.Add(ReportTable);
        }
        ...
        public MemoryStream GenerateReport()
        {
            ...
            string Filename = ReportFilename == null ? "Print" : ReportFilename;
    ...
    public class ContentCell
    {
        ...
        public ContentCell(TextWithFormat[] FormattedHeaderLine, ParagraphAlignment ParagraphAlignment)
        {
            ...
            foreach (var ft in FormattedHeaderLine)
            {
                ...
                Font font = new Font();
                formattedText.AddText(ft.Content);
                font.Bold = ft.TextFont.Bold;
                font.Italic = ft.TextFont.Italic;
                font.Size = ft.TextFont.Size;
                font.Underline = ft.TextFont.Underline;
                paragraph.AddFormattedText(ft.Content, font);
            }
            paragraph.Format.Alignment = ParagraphAlignment;
            this.Paragraph = paragraph;
        }
        public ContentCell(TextWithFormat[] FormattedHeaderLine, Unit Indent, ParagraphAlignment ParagraphAlignment)
        {
            Paragraph paragraph = new Paragraph();
            foreach (var ft in FormattedHeaderLine)
            {
        ...
        public ContentCell(TextWithFormat[] FormattedHeaderLine, int CellIndex, ParagraphAlignment HorizontalAlign, VerticalAlignment VerticalAlign, int MergeRight, int MergeDown, bool? BottomRule, Color FontColor, bool? RightRule, bool? TopRule, bool? LeftRule)
        {
            Paragraph paragraph = new Paragraph();
            foreach (var ft in FormattedHeaderLine)
            {
            ...
            paragraph.Format.Alignment = HorizontalAlign;
            this.Paragraph = paragraph;
            this.Content = Content;
            this.CellIndex = CellIndex;
            this.HorizontalAlign = HorizontalAlign;
        ...
        public ContentCell(TextWithFormat[] FormattedHeaderLine, int CellIndex, ParagraphAlignment HorizontalAlign, VerticalAlignment VerticalAlign)
        {
            Paragraph paragraph = new Paragraph();
            foreach (var ft in FormattedHeaderLine)
            {
                ...
                font.Size = ft.TextFont.Size;
                font.Underline = ft.TextFont.Underline;
                font.Superscript = ft.TextFont.Superscript;
                paragraph.AddFormattedText(ft.Content, font);
                paragraph.Format.LineSpacingRule = LineSpacingRule.Exactly;
                paragraph.Format.LineSpacing = new Unit(0.20, UnitType.Inch);
            }
            paragraph.Format.Alignment = HorizontalAlign;
            this.Paragraph = paragraph;
            this.Content = Content;
        ...
        public ContentCell(TextWithFormat[] FormattedHeaderLine, int CellIndex, ParagraphAlignment HorizontalAlign, VerticalAlignment VerticalAlign, int MergeRight)
        {
            Paragraph paragraph = new Paragraph();
            foreach (var ft in FormattedHeaderLine)
            {
    ...
    public class ContentTable
    {
        ...
        public Table AddTableRow(Table ReportTable, List<ContentCell> Cells, double RowHeight)
        {
            ...
            {
                var content = row.Paragraph == null ? row.Content : row.Paragraph.ToString();
                ...
                if(row.Content == null)
                {
                    _tableRow.Cells[row.CellIndex].Add(row.Paragraph);
                }
                else
                {
                    _tableRow.Cells[row.CellIndex].AddParagraph(row.Content);
                }
        ...
        public Table AddTableRowFormatted(Table ReportTable, ContentCell Cells, double RowHeight)
        {
            ...
            _tableRow.Cells[0].Format.LineSpacingRule = LineSpacingRule.Single;
